...
export class Snake extends AcGameObject{
    constructor(info, gamemap){
        ...
        this.dr = [-1,0,1,0];//四个方向在行的偏移量
        this.dc = [0,1,0,-1];//四个方向在列的偏移量

        this.step = 0;//表示回合数
        this.eps = 1e-2;//表示允许的误差，小于这个值即认为接近

        this.eye_direction = 0;//左下角的蛇初始朝上，右上角的蛇初始朝下
        if(this.id === 1)this.eye_direction = 2; 

        this.eye_dx = [ //蛇眼镜不同方向的x的偏移量
            [-1,1],
            [1,1],
            [1,-1],
            [-1,-1],
        ];
        this.eye_dy = [ //蛇眼镜不同方向的y的偏移量
            [-1,-1],
            [-1,1],
            [1,1],
            [-1,1],
    ...
    render(){
        ...
        for(let i = 0;i < 2; i++){
            ...
            ctx.beginPath();
            ctx.arc(eye_x, eye_y , L*0.05, 0, Math.PI * 2);//五个参数：起始点x坐标，起始点y坐标，圆的半径，圆的开始弧度和结束弧度
            ctx.fill();
        }
    }
}